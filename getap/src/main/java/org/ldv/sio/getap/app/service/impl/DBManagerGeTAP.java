...
public class DBManagerGeTAP implements IFManagerGeTAP {
  ...
  /*
   * Ancienne requête, la nouvelle ne marche pas chez moi un modif de la base de
   * données ??
   * 
   * 
  ...
  public List<HashMap<String, String>> getParameter() {
    return this.jdbcTemplate.query("SELECT keyName, keyValue FROM parameter",
  ...
  public void setDateEnvoiMail(int idDvctap) {
    ...
    // Nouvelle entrée.
    this.jdbcTemplate.update(
        "insert into sendmail(idDvctap, dateEnvoi) values(?,?)", new Object[] {
            idDvctap, dateFormatEnvoi.format(dateEnvoi) });
  }

...
public class ScheduledJobs {
	...
	public void notifyMail() throws ParseException {
		...
		String subjectMail = "[GeTAP] [" + dateFormatEnvoi.format(dateEnvoi)
		...
		for (ProfStats statsProf : lesProfStats) {
			...
			if (statsProf.getDctapattente() > 0) {
				...
				if (professeur.getMail() != null) {
					...
					for (int i = 0; i < lesDvctapProf.size(); i++) {
						...
						if (dvctapEtat == 0 || dvctapEtat == 4
							...
							int countMail = manager
									.getCountDateEnvoiMail(lesDvctapProf.get(i)
											.getId().intValue());
							...
							if (countMail == 0) {
								nouvellesDemandes += "- ["
										+ lesDvctapProf.get(i).getAccPers()
												.getNom()
										+ "] ["
										+ dateFormatEnvoi.format(lesDvctapProf
												.get(i).getDateAction())
										+ "] "
							...
							manager.setDateEnvoiMail(lesDvctapProf.get(i)
									.getId().intValue());
						}
					}
					...
					contentMail += "- Demandes validées : "
							+ statsProf.getDctapvalide() + "\n";
					contentMail += "- Demandes en attente : "
							+ statsProf.getDctapattente() + "\n";
					contentMail += "- Demandes refusées : "
							+ statsProf.getDctaprefuse() + "\n";
					contentMail += "- Demandes totales : "
							+ statsProf.getCountap() + "\n\n";

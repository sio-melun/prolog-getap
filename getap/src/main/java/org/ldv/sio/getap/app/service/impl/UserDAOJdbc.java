...
public class UserDAOJdbc implements IFUserDAO {
	...
	public List<User> getAllProfInter() {
		return jdbcTemplate
				.query("select * from user where hash <> '' AND role = 'prof-intervenant'",
						new UserMapper());
	}

	public List<User> getAllProfPrinc() {
		return jdbcTemplate
				.query("select * from user where hash <> '' AND role = 'prof-principal'",
						new UserMapper());
	}
	...
	/**
	 * rétablit le mot de passe initial de l'utilisateur (écrase l'ancien dans
	 * le cas où il aurait été modifié par l'utilisateur)
	 */
	public void updatePass(User user) {
		...
		jdbcTemplate.update("update user set hash = ? where id = ?",
				new Object[] { hash, id });

	}

	public void updateMailUser(User user) {
		Long id = user.getId();
		String mail = user.getMail();

		jdbcTemplate.update("UPDATE user SET mail = '" + mail + "' WHERE id='"
				+ id + "'");
	}

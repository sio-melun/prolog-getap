...
public class DvctapDAOJdbc implements IFDvctapDAO {
  ...
  private static final class AccMapper implements RowMapper<AccPersonalise> {
    public AccPersonalise mapRow(ResultSet rs, int rowNum) throws SQLException {
  ...
  public List<DemandeValidationConsoTempsAccPers> getAllDVCTAPByEleve(User eleve) {
    Long id = eleve.getId();
    return this.jdbcTemplate.query("select * from dctap where idEleve = " + id,
        new DemandeMapper());
  }
  ...
  public List<DemandeValidationConsoTempsAccPers> getAllDVCTAPByProfInterv(
    ...
    return this.jdbcTemplate.query(
        "select * from dctap where anneeScolaire = '"
            + UtilSession.getAnneeScolaireInSession() + "' AND idProf = " + id,
        new DemandeMapper());
  }
  ...
  public List<DemandeValidationConsoTempsAccPers> getAllDVCTAPByClasse(
    ...
    return this.jdbcTemplate
        .query(
            "select * from dctap d, user u, classe c where d.anneeScolaire = '"
  ...
  public int getAllDVCTAPByEtat(int etat, Long id) {
    int count = this.jdbcTemplate.queryForInt(
        "select count(id) from dctap where anneeScolaire = '"
            + UtilSession.getAnneeScolaireInSession()
            + "' AND Etat = ? and (idProf = ? or idEleve = ?)", new Object[] {
            etat, id, id });
    return count;
  }
  ...
  public DemandeValidationConsoTempsAccPers getDVCTAPById(Long id) {
    return this.jdbcTemplate.queryForObject("select * from dctap where id = ?",
        new Object[] { id }, new DemandeMapper());
  }
  ...
  public void addDVCTAP(DemandeValidationConsoTempsAccPers dctap) {
    ...
    this.jdbcTemplate
        .update(
            "insert into dctap(anneeScolaire, dateAction, dureeAP, Etat, idProf, idEleve, idAP) values(?,?,?,?,?,?,?)",
            new Object[] { anneeScolaire, dateAction, dureeAP, etat, idProf,
                idEleve, idAP });
  ...
  public void updateDVCTAP(DemandeValidationConsoTempsAccPers dctap) {
    ...
    this.jdbcTemplate
        .update(
            "update dctap set anneeScolaire = ?, dateAction = ?, dureeAP = ?, Etat = ?, idProf = ?, idEleve = ?, idAP = ? where id = ?",
            new Object[] { anneeScolaire, dateAction, dureeAP, etat, idProf,
                idEleve, idAP, id });
  ...
  public boolean deleteDVCTAPById(Long id) {
    int result = this.jdbcTemplate.queryForInt(
        "select count(id) from dctap where id = ?", new Object[] { id });
    if (result == 0)
      return false;
    else
      return true;
